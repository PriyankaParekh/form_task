...
function StepsComponent({ currentPage, totalPages, config }) {
  const steps = Array.from({ length: totalPages }, (_, i) => i + 1);

  const isStepOptional = (stepIndex) => {

    if (stepIndex === totalPages - 1) {
      return false; 
    }
    const stepName = config?.steps[stepIndex]?.step;
    const fields = config?.fields[stepName];
    return fields?.every((field) => !field.required);
  };
  
  return (
    <div className="py-8 stepsComponentMainDiv">
      <div className="flex flex-wrap justify-center">
        {steps?.map((step) => (
          <div key={step} className="text-center mx-auto">
            {step < currentPage ? (
              ...
              <div className="flex justify-center items-center">
                ...
                <p className="steps">Step {step}</p>
                {isStepOptional(step - 1) ? (
                  <p className="stepss text-slate-700 font-medium">Optional</p>):(null)}
                  </div>
              </div>
            )}
          </div>
